...
class Trainer:
    def __init__(self, configs, model_config, data, model, device):
        # load configs
    ...
    def get_predictions(self):
        """
        Get predictions on the test dataset and return them.
        """
        self.model.eval()
        label_preds = torch.tensor([]).to(self.device)  # 确保在正确的设备上初始化

        with torch.no_grad():
            for feature, label, sensitive_label in self.loader["test"]:
                feature = feature.to(self.device)
                label = label.to(self.device)

                class_log_prob, _, _ = self.model(
                    feature=feature,
                ...
                ypred = class_log_prob.argmax(dim=1)
                label_preds = torch.cat((label_preds, ypred))

        return label_preds.cpu().numpy().astype(int)  # 转换为整数类型
     '''
    def get_predictions(self):
        """
        获取测试集上的预测值。
        """
        self.model.eval()
        label_preds = torch.tensor([]).to(self.device)  # 确保初始化在正确的设备上

        with torch.no_grad():
            for batch_idx, (feature, label, sensitive_label) in enumerate(self.loader["test"]):
                feature = feature.to(self.device)
