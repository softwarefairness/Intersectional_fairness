...
for j in ['lr','rf','svm','dl']:
    for name in ['default', 'rew', 'eop','fairsmote', 'maat','fairmask', 'fairmutation', 'rewfairmutation', 'eopfairmutation', 'fairsmotefairmutation', 'maatfairmutation']:
...
for i in ['spd', 'aod', 'eod']:
    fout.write(i + '\n')
    fout.write('\tWin\tTie\tLose\n')
    for name in ['rewfairmutation','advfairmutation','eopfairmutation','fairsmotefairmutation','maatfairmutation']:
        count={}
        count['win'] = 0
        count['tie'] =0
        count['lose']=0
        for dataset in ['adult', 'compas', 'default', 'mep1', 'mep2']:
            for j in ['lr', 'rf', 'svm', 'dl']:
               if mann(data[j][dataset][i]['fairmutation'], data[j][dataset][i][name]) >= 0.05:
                   count['tie']+=1
               elif mean(data[j][dataset][i]['fairmutation']) < mean(data[j][dataset][i][name]):
                   count['win']+=1
               else:
                   count['lose']+=1
        fout.write(name+'\t'+str(count['win'])+'\t'+str(count['tie'])+'\t'+str(count['lose'])+'\n')
