...
approach_list = ['ifmutation', 'ifmutation2']
data = {}
...
data_key_value_used = {1:'Acc', 2: 'Mac-P', 3: 'Mac-R', 4: 'Mac-F1', 5: 'MCC', 41: 'SPD', 50: 'AOD', 59:'EOD'}
for j in ['lr','rf','svm','dl']:
    for name in ['default','ifmutation','ifmutation2']:
            ...
            for i in ['SPD','AOD','EOD']:
                if mean(data[j][dataset][i][name]) < mean(data[j][dataset][i]['default']) and mann(data[j][dataset][i][name], data[j][dataset][i]['default']) < 0.05:
                    ...
                    if abs(cliffs_delta(data[j][dataset][i][name], data[j][dataset][i]['default'])[0]) >=0.428:
                        countt['sig_large']+=1
    fout.write('\t%f\t%f\n' % (countt['sig']/12,countt['sig_large']/12))

fout.write("\n\n-------Results for Table6\n")
for i in ['SPD','AOD','EOD']:
    fout.write(i+'\n')
    fout.write('\tWin\tTie\tLose\n')
    for name in ['ifmutation2']:
        count={}
        count['win'] = 0
        count['tie'] =0
        count['lose']=0
